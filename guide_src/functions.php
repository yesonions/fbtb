...
function the_category_unlinked($separator = ' ') {
    ...
    foreach($categories as $category) {    // concate
        $thelist .= $separator . $category->name;
    }
...
function catch_that_image() {
	...
	if(empty($first_img)){ //Defines a default image
		...
		$first_img = $matches [1] [0];
		$first_img = str_replace("http://www.youtube.com/embed/", "", $first_img);
		$first_img = "http://img.youtube.com/vi/".$first_img."/0.jpg";
//		$url = $first_img;
//		parse_str( parse_url( $url, PHP_URL_QUERY ), $my_array_of_vars );
//		$first_img = $my_array_of_vars['v'];    
		  // Output: C4kxS1ksqtw
		
	}
//	$first_img=str_replace("m.jpg","z.jpg",$first_img  );
	return $first_img;
}
